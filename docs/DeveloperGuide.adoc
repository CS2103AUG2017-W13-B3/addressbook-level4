...
These commands require access to `Storage` from `Logic` and this is accomplishing by posting an event to `EventsCenter`. `Subscribers` in `StorageManager` will handle these events and respond correspondingly.
The following shows a sequence diagram, Figure 3.1.2, of how the `BackupCommand` is carried out.
