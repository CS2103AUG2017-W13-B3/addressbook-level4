= ABC Business Contact - Developer Guide
:toc:
:toc-title:
...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command). This is illustrated by Figure 3.1.2.

image::UndoRedoStartingStackDiagram.png[width="800"]
_Figure 3.1.2 : Delete Command Stack Diagram_

As the user continues to use the program, more commands are added into the `undoStack`. For example, the user may execute `add n/David ...` to add a new person. This is shown in Figure 3.1.3.

image::UndoRedoNewCommand1StackDiagram.png[width="800"]
_Figure 3.1.3 : Add Command Stack Diagram_
...
As can be seen from Figure 3.1.4, the most recent command is popped out of the `undoStack` and pushed back to the `redoStack`. The address book is then restored to the state before the `add` command executed.

image::UndoRedoExecuteUndoStackDiagram.png[width="800"]
_Figure 3.1.4 : Execute Undo Stack Diagram_

[NOTE]
If the `undoStack` is empty, then there are no other commands left to be undone, and an `Exception` will be thrown when popping the `undoStack`.

The following sequence diagram, Figure 3.1.5, shows how the undo operation works:

image::UndoRedoSequenceDiagram.png[width="800"]
_Figure 3.1.5 : Undo/Redo Sequence Diagram_
...
The user now decides to execute a new command, `clear`. As before, `clear` will be pushed into the `undoStack`. The `redoStack` is no longer empty. It will be purged as it no longer makes sense to redo the `add n/David` command (this is the behavior that most modern desktop applications follow). This is shown in Figure 3.1.6.

image::UndoRedoNewCommand2StackDiagram.png[width="800"]
_Figure 3.1.6 : Clear Command Stack Diagram _

Commands that are not undoable are not added into the `undoStack`. For example, `list`, which inherits from `Command` rather than `UndoableCommand`, will not be added after execution, as shown in Figure 3.1.7.

image::UndoRedoNewCommand3StackDiagram.png[width="800"]
_Figure 3.1.7 : List Command Stack Diagram_

The following activity diagram, Figure 3.1.8,  summarizes what happens inside the `UndoRedoStack` when a user executes a new command:

image::UndoRedoActivityDiagram.png[width="200"]
_Figure 3.1.8 : Undo/Redo Activity Diagram_
...
The sequence diagram below (Figure 3.1.2) shows how the `BackupCommand` is carried out.

image::BackupCommandSequenceDiagram.png[width="800"]
_Figure 3.2.1 : Backup Command Sequence Diagram_
...
_Figure 3.3.1 : AddTag Command Sequence Diagram_
...
Note that all `person` instances in the existing list satisfy a Predicate `currentPredicate`. Given a new Predicate `newPredicate`, filtering on the existing list is equivalent to selecting `person` instances that satisfy both `currentPredicate` and `newPredicate`. From Figure 3.4.1, it can also be viewed as the intersection of two lists of `person` objects, each satisfying one of the two predicates respectively.
...
The design for filtering the existing list applies the [https://en.wikipedia.org/wiki/Open/closed_principle[Open/Close Principle]].
...
_Figure 3.5.1: Sequence Diagram for SyncCommand_
