...
Firstly, the command posts an `AuthorizationEvent` event to the EventCenter, which is then handled by a `OAuth` class. The user is redirected to login to Google via his default browser. A callback is then done to create a GoogleAuthorizationCodeFlow object which is then used to access a user's contacts. The following diagram shows the entire sequence.

//Insert Image

An asynchronous thread has to be started for the Google authorization and fetching of Contact data, as it will otherwise block the application, which causes it to freeze. Hence, threading allows for the application to run in the background, while the user can perform other tasks. Below is the implementation of `SyncCommand`.
